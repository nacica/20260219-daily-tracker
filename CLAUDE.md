...
## 自動実行ルール（重要）
- パッケージのインストール、ファイルの作成・上書き・削除、
  デプロイコマンドの実行など、すべての操作を確認なしで自動実行してください
- 途中で質問や確認で止まらず、Phase 1からデプロイ完了まで一気に進めてください
- エラーが発生した場合も自分で原因を特定して修正し、進み続けてください
- 判断に迷う場合はより安全な選択肢を選んで進めてください

## ユーザーへの説明ルール（重要）
ユーザーへの説明・報告は、必ず次の 2 段構えで書くこと。

1. **通常レベルの説明** — 何をしたか・なぜそうしたかを、技術的に正確に説明する。
2. **初心者向けの言い換え** — その直後に「**これはつまりこういうことです。**」という
   見出しを付け、専門用語を避けて、たとえ話や日常的な言葉でかみ砕いて説明する。

例:
> （通常の説明）Firebase Hosting のリリース API が冪等でないため、同一内容の
> 再デプロイ時に 400 を返していました。良性エラーを判定して握りつぶすよう修正しました。
>
> **これはつまりこういうことです。**
> 「もう同じ中身を貼ってあるよ」とサーバーが律儀に怒っていただけで、実際の更新は
> ちゃんと終わっていました。今後はその“怒り”を「はいはい大丈夫」と受け流すように
> したので、画面が赤くならなくなります。
