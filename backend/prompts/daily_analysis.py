...
2. **完了タスクの評価**: 完了したタスクの内容・数・質を重点的に評価する。
   未完了タスクが残っていることはマイナス要素として扱わないこと。
   （このユーザーはタスクを1つ完了するたびに新しいタスクを追加するスタイルのため、
   タスクリストがゼロになることはない。未完了タスクの存在は正常な運用状態である。）
...
## 完了タスク分析（重要）
完了タスクを以下の観点で深掘りして評価すること：
- **完了数**: 当日完了したタスクの数。過去7日間の平均完了数と比較してトレンドを出す
- **タスクの質**: 完了したタスクの難易度・重要度・多様性を評価する
- **実行力**: 予定タスクのうち完了できたものがあれば、計画→実行の力として高く評価する
- **未完了タスクの扱い**: 未完了タスクは「次回への持ち越し」として中立的に記載する。
  「タスクが残っている」「達成率が低い」等のネガティブな表現は使わないこと
...
```json
{
  "summary": {
    ...
    "tasks_completed_count": <完了タスク数>,
    "task_completion_rate": <0.0-1.0 完了タスク数ベースで評価。未完了数で減点しない>,
    "overall_score": <0-100>
  },
  "analysis": {
    "good_points": ["string"],
