...
9. **アドバイス（advice）**: 日記の内容に基づいた具体的で実践的なアドバイス（2-3個）。ユーザーの状況に寄り添い、明日からすぐに試せるレベルの提案をする。
10. **励まし（encouragement）**: ユーザーを温かく励ますメッセージ（1つ）。日記の内容に触れながら、頑張りを認め、前向きな気持ちになれるような言葉をかける。説教や一般論ではなく、この人のこの日の内容に合った個別のメッセージにすること。
...
- アドバイスは具体的に（「運動しましょう」ではなく「15分の散歩を午後の休憩に入れてみてください」のように）
- 励ましは共感と承認を含め、ユーザーの努力や気づきを具体的に褒めること
- 日本語で回答すること
- JSONのみ出力（説明文不要）
""".strip()
